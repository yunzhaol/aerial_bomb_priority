...
cleaned_aerial_priority <-
  ...
  mutate(
    ...
    # Replace NA with "others" in 'country_mission' and make sure it's a character
    country_mission = if_else(is.na(country_mission), "others", as.character(country_mission)),
  ...
  mutate(
    country_mission = factor(country_mission),
    tgt_industry = factor(tgt_industry),
    # Convert 'tgt_priority' to an ordered factor
    tgt_priority = factor(tgt_priority, levels = c("target of last resort", "target of opportunity", "secondary target", "primary target"), ordered = TRUE)
  ) %>%
  filter(complete.cases(tgt_priority, tgt_industry, country_mission, bomb_tons, aircraft_attack))
...
# country_mission: Represents the country executing the flying mission, reflecting which nation's air force units participated in the specific bombing.
# bomb_tons: Represents the total tons of bombs dropped during a mission, reflecting the attack intensity on the target.
# aircraft_attack: Represents the total number of aircraft involved in the attack, which can be used to measure the scale and force of an attack.
